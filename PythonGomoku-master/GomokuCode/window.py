from PyQt5.QtWidgets import QMainWindow, QMessageBox, QDialog, QVBoxLayout, QRadioButton, QPushButton, QButtonGroup, QLabel
...
class FirstPlayerDialog(QDialog):
    """选择先手的对话框"""
    
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle('选择先手')
        ...
        # 添加提示标签
        label = QLabel('请选择谁先落子：')
        layout.addWidget(label)
        
        # 创建单选按钮组
        self.radio_human = QRadioButton('人类玩家先手')
        self.radio_computer = QRadioButton('电脑先手')
        self.radio_human.setChecked(True)  # 默认人类先手
        
        layout.addWidget(self.radio_human)
        layout.addWidget(self.radio_computer)
    ...
    def is_human_first(self):
        """返回True表示人类先手，False表示电脑先手"""
        return self.radio_human.isChecked()
...
class GomokuWindow(QMainWindow):
    ...
    def game_start(self):
        """游戏开始前的初始化，弹出选择先手对话框"""
        dialog = FirstPlayerDialog(self)
        if dialog.exec_() == QDialog.Accepted:
            human_first = dialog.is_human_first()
            # 根据用户选择初始化游戏
            self.g = Gomoku(human_first=human_first)

            if human_first == False:
                self.g.ai_move_indicated_1step()
            self.repaint()  # 刷新界面显示空棋盘
